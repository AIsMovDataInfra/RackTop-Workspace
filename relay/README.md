...
部署日期：2026-09-08（北京时间）。

公网中继已部署到 Evoxt VM 908342。该服务提供经过身份验证的双向 WebSocket 字节通道；RackTop 1.26.0-linux.5 已接入共享协议，访客需要使用包含本功能的 RackTop 客户端。
...
已完成：正式 IP TLS 证书、Nginx、低权限中继、主人鉴权、分角色连接令牌、双向传输、容量与流量限制、过期及断开清理、开机启动、证书自动续期。

服务仅连接已认证的 host/guest 配对，不接受目的主机、SSH 命令或任意代理地址。服务日志不记录业务数据、请求正文或令牌；公网健康接口只暴露运行状态。
...
- `GET /v1/pending`：主人读取 `{tickets:[...]}` 并更新在线心跳；客户端每秒轮询。
- `POST /v1/routes/:id/connect`：访客以该路由的 Bearer 令牌请求新房间；主人离线超过 10 秒则拒绝分配。返回 guest ticket，host ticket 经 pending 提供。
...
0.2.0 源码测试在本机 Node 20 和服务器 Node 22.22.1 均为 27/27 通过，覆盖身份验证、角色抢占、过期清理、二进制顺序、限制、慢连接及控制帧边界。
...
证书首次申请演练和自动续期演练均成功，续期时的 Nginx reload hook 已实际执行。RackTop 1.26.0-linux.5 已通过实际公网中继访问 A100 的监控、终端、262181 字节文件往返与 SHA-256 校验、同设备重连、撤销后断开并拒绝重连。两端 Rust 客户端运行在同一电脑上，经真实公网中继；尚未进行两位同事、两台不同网络电脑的实机验收，也未进行 VM 整机重启测试。
