# AIsMov RackTop 团队工作台 API（2.9.0）

**版本范围：本文对应 2.9.0 当前源码。2.9.0 新增仅超级管理员可用的服务器连接失败按小时聚合日志；正式部署、桌面包和公开镜像状态以本轮版本信息为准。**
...
`POST /api/servers/import` 仅允许当前仍有效的超级管理员，在 `account` 模式使用。沿用同源、CSRF／设备令牌和 64 KiB JSON 请求体限制，不接受原始文件。网页在本机解析 SSH 配置，只提交以下连接元数据：
...
```json
{
  ...
  "memberIds": [],
  "servers": [
    {"name": "训练节点", "host": "gpu.example.test", "port": 22, "username": "researcher", "jump": null, "enabled": true}
  ]
}
```

`servers` 为 1–50 条；名称最多 24 个字符，端口为 1–65535，地址和用户名沿用单条创建校验。`jump` 为 null 或 `{host,port,username}`，不支持多跳；`enabled` 缺省 true。未知字段（包括密码、私钥和任意 SSH 指令）拒绝。授权成员须仍属于目标组织，最多 128 人；普通成员及组织管理员调用返回 403。
...
| `category` | 新建必选，值只能为 `机械臂`、`台式主机`、`笔记本电脑`、`显示屏`、`摄像头模组`、`实验物料`、`小推车`、`夹爪`。设备申请使用相同类别值；英文界面将 `笔记本电脑` 显示为 Laptop，API 值仍为中文。 |
| `model` | 可选，最多 160 字符。 |
...
| 请求 | 输入／权限 |
| --- | --- |
| `GET /api/resources` | `{resources:Resource[]}`；返回有效目录绑定及当前 SSH 授权允许读取的本公司硬件资源，超管可跨公司读取有效绑定；手工资源沿用组织规则。未绑定的旧硬件不返回。 |
...
### 服务器连接失败日志（2.9.0，仅超级管理员）

桌面客户端在超级管理员账号下连接启用的组织服务器失败时，最多按每台服务器每个 UTC 小时上报一次；同一小时内的重复失败由服务端继续聚合计数。上报使用设备 Bearer 会话和当前 `X-RackTop-Company`，网页 Cookie、普通管理员和普通成员设备均不能写入。目录版本变化、服务器停用、账号权限变化或组织切换会拒绝旧上报。

```text
POST /api/servers/:id/connectivity-failures
{serverVersion,reason}
```

`serverVersion` 必须是当前服务器目录版本；`reason` 只能为 `timeout`、`authentication`、`host_key`、`ssh_start`、`remote_command`、`credentials` 或 `unknown`。客户端只发送分类原因，不发送原始 SSH 错误、完整命令、主机地址、SSH 用户名、密码、私钥、令牌或环境变量。服务端响应为 `{ok:true,hourAt}`。

超级管理员网页通过 `GET /api/admin/server-connectivity-log` 查看最近 30 天的记录，最多返回 500 条，按小时倒序；同一服务器同一小时最多一条，字段为 `{serverId,serverName,company,hourAt,firstFailedAt,lastFailedAt,failureCount,reason}`。服务端保留 180 天后清理，日志读取不会返回主机地址、SSH 用户名或认证材料。普通管理员、普通成员、匿名会话和未登录设备均拒绝读取。
