...
export function WeeklyStatisticsView({ members, state, busy, paused, refreshKey, onOpen, onDenied }: Props) {
  ...
  const memberChoices = new Map(members.map(member => [member.id, { id: member.id, name: member.name, company: member.company }]))
  // Include report snapshots for former members, while keeping the selected name visible during refresh.
  for (const row of result?.data.rows || []) memberChoices.set(row.authorId, { id: row.authorId, name: row.name, company: row.company })
  ...
  return <section className="weekly-statistics" aria-label={t('周报统计', 'Weekly statistics')}>
    ...
    <p className="field-help">{t('按所选周及筛选范围统计。应报包含该周结束前注册的在册成员，以及该周已有报告的历史作者。已有周报沿用报告中的公司，缺报按成员当前公司统计。', 'Counts follow the selected week and filters. Expected members include active members registered by week-end and all authors with a report that week. Existing reports use their recorded company; missing reports use the member’s current company.')}</p>
