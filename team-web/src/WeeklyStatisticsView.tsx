...
import { COMPANY_OPTIONS, memberCompanies, type Company, type Member } from './types'
...
export function WeeklyStatisticsView({ members, state, busy, paused, refreshKey, onOpen, onDenied }: Props) {
  ...
  const memberChoices = new Map(members.filter(member => !member.isSuperAdmin).map(member => [member.id, { id: member.id, name: member.name, companies: new Set<Company | null>(memberCompanies(member).length ? memberCompanies(member) : [null]) }]))
  // Include report snapshots for former members, while keeping the selected name visible during refresh.
  for (const row of result?.data.rows || []) if (!superAdminIds.has(row.authorId)) {
    const choice = memberChoices.get(row.authorId)
    if (choice) choice.companies.add(row.company)
    else memberChoices.set(row.authorId, { id: row.authorId, name: row.name, companies: new Set([row.company]) })
  }
  const eligibleMembers = [...memberChoices.values()].filter(member => !company || (member.companies.has(company === 'unassigned' ? null : company)))
  ...
  return <section className="weekly-statistics" aria-label={t('周报统计', 'Weekly statistics')}>
    ...
    <p className="field-help">{t('超级管理员不参与统计。按所选周及筛选范围统计，应报包含该周结束前注册的在册成员，以及该周已有报告的历史作者。已有周报沿用报告中的公司，每位成员的每个所属组织分别统计应报与缺报。', 'Super administrators are excluded. Counts follow the selected week and filters; expected members include active members registered by week-end and all authors with a report that week. Existing reports use their recorded company; each member’s current organizations are counted separately for expected and missing reports.')}</p>
