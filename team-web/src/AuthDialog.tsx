...
export function AuthDialog({ session, bootstrapToken, onClose, onSignedIn, t, embedded = false }: { session: Session; bootstrapToken?: string; onClose: () => void; onSignedIn: (session: Session) => void; t: Translate; embedded?: boolean }) {
  ...
  async function submit(event: React.FormEvent) {
    ...
    if (!username.trim()) { setError(new Error(t('请输入成员名称', 'Enter a member name'))); return }
    ...
    try {
      ...
      const result = register ? await api.register({ name: username.trim(), password, rememberMe, ...(bootstrapToken ? { bootstrapToken } : {}) }) : await api.login({ username: username.trim(), password, rememberMe })
      setPassword(''); onSignedIn(result)
    } catch (reason) { setError(reason) } finally { setBusy(false) }
  }
  const message = error instanceof ApiError && error.status === 401 ? t('成员名称或密码不正确，请重新输入。', 'The member name or password is incorrect. Please try again.') : errorText(error, t)
  return <AuthFrame embedded={embedded} title={title} subtitle={recover ? t('提交成员名称，由超级管理员协助重置密码。', 'Submit your member name so the super administrator can help reset your password.') : t('登录后查看团队设备、照片和排期。', 'Sign in to view team equipment, photos and schedules.')} onClose={onClose} busy={busy} t={t}>
    {session.authMode === 'account' ? <form onSubmit={(event) => void submit(event)}>
      <div className="dialog-body">
        ...
        <label>{t('成员名称', 'Member name')}<input name="username" type="text" autoComplete="username" autoCapitalize="none" spellCheck={false} required disabled={busy} value={username} onChange={(event) => { setUsername(event.target.value); setRecoverySent(false) }} /></label>
        <p className="field-help">{register ? t('这个名称同时用于登录和团队中的显示，注册后固定使用。', 'This fixed name is used both to sign in and throughout the team.') : t('旧账号仍可使用原用户名登录。', 'Existing accounts can still sign in with their original username.')}</p>
