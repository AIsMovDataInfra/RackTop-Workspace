...
export function AuthDialog({ session, bootstrapToken, onClose, onSignedIn, t }: { session: Session; bootstrapToken?: string; onClose: () => void; onSignedIn: (session: Session) => void; t: Translate }) {
  ...
  return <Dialog compact title={title} subtitle={t('无需登录即可查看设备和排期，登录后可登记设备和预约。', 'View devices and schedules freely. Sign in to register devices and make reservations.')} onClose={onClose} busy={busy} t={t}>
