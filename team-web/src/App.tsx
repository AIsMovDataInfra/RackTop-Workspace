...
export default function App() {
  ...
  if (session) return <Workspace session={session} state={state} bootstrapToken={bootstrapToken} onSessionChanged={(value) => { setSession(value); if (value.user) setBootstrapToken(undefined) }} onSessionExpired={() => void load()} onLogout={async () => { await api.logout(); await load() }} />
