...
const fields = ['name', 'category', 'model', 'responsiblePerson', 'currentUser', 'location', 'notes', 'status'] as const
const emptyDraft: EquipmentDraft = { name: '', category: '', model: '', responsiblePerson: '', currentUser: '', location: '', notes: '', status: 'available' }
...
export function EquipmentWorkspace({ id, session, state, navigate, onSessionChanged, onSessionExpired, onLogout, bootstrapToken }: { id?: string; session: Session; state: PreferencesState; navigate: (path: string) => void; onSessionChanged: (session: Session) => void; onSessionExpired: () => void; onLogout: () => Promise<void>; bootstrapToken?: string }) {
  ...
  function openEditor(kind: Editor['kind']) {
    ...
    setEditor({ kind, base: kind === 'create' ? undefined : item, draft: kind === 'create' ? { ...emptyDraft, responsiblePerson: session.user.name } : { ...draftOf(item!), ...(kind === 'assign' ? { currentUser: session.user.name, status: 'in_use' as const } : kind === 'return' ? { currentUser: '', status: 'available' as const } : {}) } })
    setModal('editor')
  }
  ...
  const filtered = items.filter((entry) => (!status || entry.status === status) && [entry.name, entry.code, entry.category, entry.model, entry.serialNumber, entry.responsiblePerson, entry.currentUser, entry.location].some((value) => value.toLocaleLowerCase().includes(search.trim().toLocaleLowerCase())))
  ...
  return <>
    <div className="workspace-shell equipment-workspace" inert={Boolean(modal)}>
      <aside className="sidebar">
        ...
        <div className="sidebar-bottom"><div className="sidebar-note"><QrCode size={17} /><p>{t('贴上设备标签，成员登录后扫码查看与登记。', 'Members can scan a label to view and register equipment after signing in.')}</p></div><button className="sidebar-settings" onClick={() => setModal('settings')}><Settings size={17} />{t('设置', 'Settings')}</button><div className="profile"><span className="profile-avatar"><UserRound size={19} /></span><div><strong>{session.user?.name || t('访客', 'Guest')}</strong><small>{session.user ? t('团队成员', 'Team member') : t('扫码即可查看', 'Scan to view')}</small></div>{session.user ? <button className="icon-button" aria-label={t('退出登录', 'Sign out')} disabled={loggingOut} onClick={async () => { setLoggingOut(true); try { await onLogout() } catch (reason) { setError(reason) } finally { setLoggingOut(false) } }}><LogOut size={16} /></button> : <button onClick={onSessionExpired}>{t('登录', 'Sign in')}</button>}</div></div>
      </aside>
      <main className="main-workspace">
        ...
        <div className="page-content">
          ...
          {loading ? <p role="status" className="loading-inline"><RefreshCw size={17} />{t('正在读取设备…', 'Loading equipment…')}</p> : id ? item && <>
            ...
            {filtered.length ? <div className="equipment-list">{filtered.map((entry) => <a className="equipment-row" href={`/equipment/${encodeURIComponent(entry.id)}`} key={entry.id} onClick={(event) => { if (event.button === 0 && !event.ctrlKey && !event.metaKey && !event.shiftKey && !event.altKey) { event.preventDefault(); navigate(`/equipment/${encodeURIComponent(entry.id)}`) } }}><span className="resource-icon"><Package size={21} /></span><div className="equipment-row-name"><small>{entry.serialNumber}</small><h3>{entry.name}</h3><span>{entry.model || categoryText(entry.category, t) || t('型号未填写', 'Model not recorded')}</span></div><div className="equipment-row-meta"><span><UserRound size={14} />{entry.responsiblePerson || t('负责人未填写', 'No responsible person')}</span><span><UserRound size={14} />{t('使用人：', 'In use by: ')}{entry.currentUser || t('未登记', 'Not assigned')}</span><span><MapPin size={14} />{locationText(entry.location, t) || t('位置未填写', 'No location')}</span></div><Status value={entry.status} t={t} /><ChevronRight size={17} /></a>)}</div> : <div className="empty-state"><span><Package size={28} /></span><h3>{items.length ? t('没有匹配的设备', 'No matching equipment') : t('还没有设备', 'No equipment yet')}</h3><p>{items.length ? t('试试其他关键词或状态。', 'Try another search or status.') : t('新增第一台设备，生成可贴在设备上的二维码标签。', 'Add your first device and create its QR label.')}</p><button onClick={() => items.length ? (setSearch(''), setStatus('')) : openEditor('create')}>{items.length ? t('清除筛选', 'Clear filters') : t('新增设备', 'Add device')}</button></div>}
          </>}
        </div>
      </main>
    </div>
    {modal === 'settings' && <SettingsDialog session={session} state={state} onClose={close} onSessionChanged={onSessionChanged} />}
    {modal === 'editor' && editor && <EquipmentEditor editor={editor} setEditor={setEditor} t={t} onClose={close} onAuthRequired={onSessionExpired} onSaved={(saved) => { setModal(null); setEditor(null); if (id !== saved.id) navigate(`/equipment/${encodeURIComponent(saved.id)}`); else setRevision((value) => value + 1); setNotice('saved') }} />}
...
export function EquipmentLabel({ equipment, t, onClose }: { equipment: Equipment; t: Translate; onClose: () => void }) {
  ...
  return <Dialog compact title={t('设备标签', 'Device label')} t={t} onClose={onClose}><div className="dialog-body"><div className="equipment-print-label"><strong>{equipment.name}</strong><span>{equipment.serialNumber}</span>{equipment.model && <small>{equipment.model}</small>}{image ? <img width="240" height="240" src={image} alt={t('扫码查看此设备', 'Scan to view this device')} /> : <p role={error ? 'alert' : 'status'}>{error ? t('二维码生成失败，请关闭后重试。', 'Could not generate the QR code. Close and try again.') : t('正在生成二维码…', 'Generating QR code…')}</p>}<small>{t('扫码查看设备信息', 'Scan for device details')}</small></div><a className="equipment-label-url" href={url}>{url}</a><p className="field-help">{t('设备信息更新后仍使用这张标签。扫码后登录团队账号查看。', 'This label keeps working after details change. Sign in with your team account after scanning.')}</p></div><footer>{image && <a className="button" href={image} download={`${equipment.serialNumber}-QR.png`}><Download size={16} />{t('下载二维码', 'Download QR')}</a>}<button className="primary" disabled={!image} onClick={() => window.print()}><Printer size={16} />{t('打印标签', 'Print label')}</button></footer></Dialog>
}
