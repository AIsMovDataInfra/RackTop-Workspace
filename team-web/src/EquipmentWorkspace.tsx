...
import { Building2, ArrowLeft, Check, ChevronRight, ClipboardList, LayoutGrid, HandHelping, LogOut, MapPin, Package, Pencil, Plus, QrCode, RefreshCw, Search, Server, Settings, UserRound } from 'lucide-react'
...
export function EquipmentWorkspace({ id, session, state, navigate, onSessionChanged, onSessionExpired, onLogout, bootstrapToken }: { id?: string; session: Session; state: PreferencesState; navigate: (path: string) => void; onSessionChanged: (session: Session) => void; onSessionExpired: () => void; onLogout: () => Promise<void>; bootstrapToken?: string }) {
  ...
  function openEditor(kind: Editor['kind']) {
    ...
    setEditor({ kind, base: kind === 'create' ? undefined : item, draft: kind === 'create' ? { ...emptyDraft, company: session.user.isSuperAdmin ? (COMPANY_OPTIONS.some(company => company === companyFilter) ? companyFilter : '') : session.user.company || '', responsiblePerson: session.user.name } : { ...draftOf(item!), ...(kind === 'assign' ? { currentUser: session.user.name, status: 'in_use' as const } : kind === 'return' ? { currentUser: '', status: 'available' as const } : {}) } })
    setModal('editor')
  }
  ...
  const companies = session.user?.isSuperAdmin ? COMPANY_OPTIONS : COMPANY_OPTIONS.filter(company => company === session.user?.company)
  const selectedCompany = session.user?.isSuperAdmin ? companyFilter : session.user?.company || ''
  const matchesCompany = (entry: Equipment) => !selectedCompany || (selectedCompany === 'unassigned' ? !entry.company : entry.company === selectedCompany)
  const companyItems = items.filter(matchesCompany)
  const companyLabel = selectedCompany === 'unassigned' ? t('待分配公司', 'Unassigned company') : selectedCompany
  const companyIsEmpty = Boolean(selectedCompany) && companyItems.length === 0
  const filtered = companyItems.filter((entry) => (!status || entry.status === status) && [entry.name, entry.code, entry.category, entry.model, entry.company || '', entry.serialNumber, entry.responsiblePerson, entry.currentUser, entry.location].some((value) => value.toLocaleLowerCase().includes(search.trim().toLocaleLowerCase())))
  ...
  return <>
    <div className="workspace-shell equipment-workspace" inert={Boolean(modal)}>
      <aside className="sidebar">
        ...
        <nav className="main-nav" aria-label={t('主导航', 'Main navigation')}><button onClick={() => navigate('/')}><LayoutGrid size={18} />{t('算力预约', 'Compute bookings')}</button><button onClick={() => navigate('/servers')}><Server size={18}/>{t('服务器资源', 'Server resources')}</button><button onClick={() => navigate('/requests')}><HandHelping size={18}/>{t('办公设备申请', 'Office equipment requests')}</button><button className="is-active" aria-current="page" onClick={() => navigate('/equipment')}><Package size={18} />{t('资产设备管理', 'Asset management')}</button>{session.user?.isSuperAdmin && <button onClick={() => navigate('/members')}><UserRound size={18} />{t('成员管理', 'Members')}</button>}</nav>
      ...
      <main className="main-workspace">
        ...
        <div className="page-content">
          ...
          {loading ? <p role="status" className="loading-inline"><RefreshCw size={17} />{t('正在读取设备…', 'Loading equipment…')}</p> : id ? item && <>
            ...
            {filtered.length ? <div className="equipment-list">{filtered.map((entry) => <a className="equipment-row" href={`/equipment/${encodeURIComponent(entry.id)}`} key={entry.id} onClick={(event) => { if (event.button === 0 && !event.ctrlKey && !event.metaKey && !event.shiftKey && !event.altKey) { event.preventDefault(); navigate(`/equipment/${encodeURIComponent(entry.id)}`) } }}><EquipmentThumbnail equipment={entry} t={t}/><div className="equipment-row-name"><small>{entry.serialNumber}</small><h3>{entry.name}</h3><span>{entry.model || categoryText(entry.category, t) || t('型号未填写', 'Model not recorded')}</span></div><div className="equipment-row-meta"><span className="equipment-row-company"><Building2 size={14} /><span>{t('公司：', 'Company: ')}{entry.company || t('待分配', 'Unassigned')}</span></span><span><UserRound size={14} />{entry.responsiblePerson || t('负责人未填写', 'No responsible person')}</span><span><UserRound size={14} />{t('使用人：', 'In use by: ')}{entry.currentUser || t('未登记', 'Not assigned')}</span><span><MapPin size={14} />{locationText(entry.location, t) || t('位置未填写', 'No location')}</span></div><Status value={entry.status} t={t} /><ChevronRight size={17} /></a>)}</div> : <div className="empty-state"><span><Package size={28} /></span><h3>{companyIsEmpty ? t(`${companyLabel}暂无设备`, `No equipment for ${companyLabel}`) : items.length ? t('没有匹配的设备', 'No matching equipment') : t('还没有设备', 'No equipment yet')}</h3><p>{companyIsEmpty ? t('可以新增设备，或由超级管理员在设备详情中调整公司归属。', 'Add a device, or ask the super administrator to update an existing device’s company.') : items.length ? t('试试其他关键词、公司或状态。', 'Try another search, company or status.') : t('新增第一台设备，生成可贴在设备上的二维码标签。', 'Add your first device and create its QR label.')}</p><button onClick={() => items.length && !companyIsEmpty ? (setSearch(''), setStatus(''), setCompanyFilter('')) : openEditor('create')}>{items.length && !companyIsEmpty ? t('清除筛选', 'Clear filters') : t('新增设备', 'Add device')}</button>{companyIsEmpty && session.user.isSuperAdmin && <button onClick={() => { setCompanyFilter(''); setSearch(''); setStatus('') }}>{t('查看全部公司', 'View all companies')}</button>}</div>}
          </>}
        </div>
      </main>
    </div>
