...
import { Activity, ArrowLeft, Check, ChevronRight, ClipboardList, LayoutGrid, LogOut, MapPin, Package, Pencil, Plus, QrCode, RefreshCw, Search, Settings, UserRound } from 'lucide-react'
  ...
  return <>
    <div className="workspace-shell equipment-workspace" inert={Boolean(modal)}>
      <aside className="sidebar">
        ...
        <nav className="main-nav" aria-label={t('主导航', 'Main navigation')}><button onClick={() => navigate('/')}><LayoutGrid size={18} />{t('资源看板', 'Resource board')}</button><button className="is-active" aria-current="page" onClick={() => navigate('/equipment')}><Package size={18} />{t('设备管理', 'Equipment')}</button>{session.user?.isSuperAdmin && <button onClick={() => navigate('/members')}><UserRound size={18} />{t('成员管理', 'Members')}</button>}</nav>
        <div className="sidebar-bottom"><div className="sidebar-note"><QrCode size={17} /><p>{t('贴上设备标签，成员登录后扫码查看与登记。', 'Members can scan a label to view and register equipment after signing in.')}</p></div><button className="sidebar-settings" onClick={() => setModal('settings')}><Settings size={17} />{t('设置', 'Settings')}</button><div className="profile"><span className="profile-avatar"><UserRound size={19} /></span><div><strong>{session.user?.name || t('访客', 'Guest')}</strong><small>{session.user?.isSuperAdmin ? t('超级管理员', 'Super administrator') : session.user?.role === 'admin' ? t('资源管理员', 'Resource administrator') : t('团队成员', 'Team member')}</small></div>{session.user ? <button className="icon-button" aria-label={t('退出登录', 'Sign out')} disabled={loggingOut} onClick={async () => { setLoggingOut(true); try { await onLogout() } catch (reason) { setError(reason) } finally { setLoggingOut(false) } }}><LogOut size={16} /></button> : <button onClick={onSessionExpired}>{t('登录', 'Sign in')}</button>}</div></div>
      </aside>
    ...
    {modal === 'editor' && editor && <EquipmentEditor isSuperAdmin={Boolean(session.user?.isSuperAdmin)} editor={editor} setEditor={setEditor} t={t} onClose={close} onAuthRequired={onSessionExpired} onSaved={(saved) => { setModal(null); setEditor(null); if (id !== saved.id) navigate(`/equipment/${encodeURIComponent(saved.id)}`); else setRevision((value) => value + 1); setNotice('saved') }} />}
    {modal === 'label' && item && <EquipmentLabel equipment={item} t={t} onClose={close} />}
  </>
}

function EquipmentEditor({ isSuperAdmin, editor, setEditor, t, onClose, onSaved, onAuthRequired }: { isSuperAdmin: boolean; editor: Editor; setEditor: React.Dispatch<React.SetStateAction<Editor | null>>; t: Translate; onClose: () => void; onSaved: (equipment: Equipment) => void; onAuthRequired: () => void }) {
