...
export function ServersWorkspace(props: Props) {
  ...
  async function load(automatic = false) {
    if (automatic && pendingLoad.current !== null) return
    const attempt = ++generation.current
    pendingLoad.current = attempt
    ...
    try { const value = await api.servers(company || undefined); if (attempt === generation.current) setServers(value.servers) }
      ...
      if (pendingLoad.current === attempt) pendingLoad.current = null
  ...
  async function save() {
    ...
    try {
      ...
      // A GET started before this save must not overwrite the confirmed update.
      if (attempt !== generation.current) return
      generation.current++; pendingLoad.current = null; setLoading(false)
      setServers(previous => [...previous.filter(item => item.id !== value.server.id), value.server])
      setEditor(null); setNotice(t('已保存，获授权成员会自动收到更新。', 'Saved. Authorized members receive updates automatically.'))
    } catch (reason) {
  ...
  return <>
    <WorkModuleFrame {...props} section="servers" title={t('服务器资源', 'Server resources')} subtitle={t('统一维护 SSH 地址、用户名和访问成员。', 'Maintain SSH addresses, usernames and member access in one place.')} modal={Boolean(editor) || importing} actions={<><button disabled={loading} onClick={() => void load()}><RefreshCw size={16}/>{t('刷新', 'Refresh')}</button>{superAdmin && <button onClick={() => { setNotice(''); setImporting(true) }}><Upload size={16}/>{t('导入 SSH 配置', 'Import SSH configuration')}</button>}{admin && <button className="primary" onClick={() => open('create')}><Plus size={17}/>{t('添加服务器', 'Add server')}</button>}</>}>
          ...
          <div className="server-directory">{filtered.map(server => <article className="server-entry" key={server.id}>
            ...
            <p className="server-help">{t('SSH 认证：', 'SSH authentication: ')}{server.hasPassword ? t('管理员共享密码', 'Password shared by administrator') : t('成员本机认证', 'Member’s local credentials')}{server.jump && <><br/>{t('跳板机认证：', 'Jump-host authentication: ')}{server.hasJumpPassword ? t('管理员共享密码', 'Password shared by administrator') : t('成员本机认证', 'Member’s local credentials')}</>}</p>
    ...
    {editor && form && <Dialog title={editor.mode === 'create' ? t('添加服务器', 'Add server') : editor.mode === 'edit' ? t('编辑服务器', 'Edit server') : t('服务器授权', 'Server access')} subtitle={editor.base?.name} onClose={() => setEditor(null)} busy={busy} t={t}>
      <form onSubmit={event => { event.preventDefault(); void save() }}><div className="dialog-body server-form">
        ...
        {editor.mode !== 'grants' && <><label>{t('组织', 'Organization')}<select required value={form.company} disabled={busy || editor.mode === 'edit' || !superAdmin} onChange={event => change({company:event.target.value as Company,memberIds:[]})}>{COMPANY_OPTIONS.filter(item => superAdmin || item === session.user?.company).map(item => <option key={item}>{item}</option>)}</select></label>
          ...
          <ServerPasswordField label={t('共享 SSH 密码', 'Shared SSH password')} configured={Boolean(editor.base?.hasPassword)} value={form.password} disabled={busy} onChange={password => change({password})} t={t}/>
          <p className="server-help">{t('填写服务器现有的登录密码，保存后分配给本服务器的获授权成员。这里不会修改服务器本身的密码。', 'Enter the server’s existing login password to share it with authorized members. This does not change the password on the server.')}</p>
          <label className="server-check"><input type="checkbox" checked={!!form.jump} disabled={busy} onChange={event => change({jump:event.target.checked ? {host:'',port:22,username:''} : null,jumpPassword:undefined})}/>{t('通过跳板机连接', 'Connect through a jump host')}</label>
          {form.jump && <fieldset><legend>{t('跳板机', 'Jump host')}</legend><div className="server-form-grid"><label>{t('主机地址', 'Host')}<input required value={form.jump.host} disabled={busy} onChange={event => change({jump:{...form.jump!,host:event.target.value}})}/></label><label>{t('端口', 'Port')}<input required type="number" min={1} max={65535} value={form.jump.port} disabled={busy} onChange={event => change({jump:{...form.jump!,port:event.target.valueAsNumber}})}/></label></div><label>{t('SSH 用户名', 'SSH username')}<input required value={form.jump.username} disabled={busy} onChange={event => change({jump:{...form.jump!,username:event.target.value}})}/></label><ServerPasswordField label={t('共享跳板机密码', 'Shared jump-host password')} configured={Boolean(editor.base?.hasJumpPassword)} value={form.jumpPassword} disabled={busy} onChange={jumpPassword => change({jumpPassword})} t={t}/></fieldset>}
          {editor.mode === 'edit' && (editor.base?.hasPassword || editor.base?.hasJumpPassword) && <p className="server-help">{t('修改主机地址、端口或用户名会清除对应的旧共享密码；如继续使用密码，请同时填写新密码。', 'Changing a host, port or username removes its saved shared password. Enter the new password in the same update to continue sharing it.')}</p>}
