...
export function WeeklyWorkspace(props: WorkModuleProps) {
  ...
  function denied(reason: unknown) {
    ...
    generation.current++; setReports([]); setMembers([]); setReport(null); setOpen(false); setTodos([]); setNextPlan(''); setHistory([]); setReviewComment(''); setScore(''); setLoading(false); setError(reason)
  ...
  useEffect(() => { setReports([]); setMembers([]); setOpen(false); setReport(null); setTodos([]); setNextPlan(''); setHistory([]); setReviewComment(''); setScore(''); if (user) void load(); return () => { generation.current++ } }, [user?.id, user?.company, user?.isSuperAdmin])
  useEffect(() => {
  ...
  function create() {
    setReport(null); setHistory([]); setWeekStart(currentWeek()); setAuthorId(user?.company ? user.id : members.find(member => member.company)?.id || '')
  ...
  async function save(status: 'draft' | 'submitted') {
    ...
    try {
      ...
      if (request !== generation.current) return
      setOpen(false); setReport(null); setNotice(status === 'draft' ? t('草稿已保存。', 'Draft saved.') : t('周报已提交，正文已锁定，等待人工评审。', 'Report submitted. Its contents are locked for manual review.')); await load()
      void value
  ...
  async function review(action: 'assign' | 'score') {
    ...
    try {
      ...
      if (request !== generation.current) return
      setReport(value.report); setReports(old => old.map(item => item.id === value.report.id ? value.report : item))
  ...
  return <>
    <WorkModuleFrame {...props} section="reports" title={t('周报与绩效', 'Reports and reviews')} subtitle={t('回顾工作、安排计划，并查看人工评审结果。', 'Review your work, plan ahead and read manual feedback.')} modal={open} actions={<><button disabled={loading || busy} onClick={() => void load()}><RefreshCw size={16}/>{t('刷新', 'Refresh')}</button><button className="primary" disabled={busy || loading} onClick={create}><Plus size={17}/>{t('写周报', 'Write report')}</button></>}>
      ...
      {loading ? <p role="status">{t('正在读取周报…', 'Loading reports…')}</p> : !error && (shown.length ? <div className="work-module-grid">{shown.map(item => <article className="work-record" key={item.id}><header><h2>{item.authorName}</h2><span className={`work-module-status work-module-status--${item.status}`}>{item.status === 'draft' ? t('草稿', 'Draft') : t('已提交', 'Submitted')}</span></header><p>{item.company} · {t('所在周', 'Week of')} {item.weekStart}</p><dl><div><dt>{t('评审人', 'Reviewer')}</dt><dd>{item.reviewerName || t('待指定', 'Unassigned')}</dd></div><div><dt>{t('人工评分', 'Manual score')}</dt><dd>{item.score == null ? '—' : `${item.score} / 100`}</dd></div></dl><footer><small className="muted">{formatTime(item.updatedAt, preferences.locale)}</small><button disabled={busy} onClick={() => void openReport(item.id)}>{t('打开周报', 'Open report')}</button></footer></article>)}</div> : <div className="empty-state"><h2>{t('暂无可查看的周报', 'No reports available')}</h2><p>{t('可以创建自己的周报；评审人仅看到被指定的记录。', 'Create your report. Reviewers see only records assigned to them.')}</p></div>)}
    </WorkModuleFrame>
    {open && <Dialog title={report ? `${report.authorName} · ${report.weekStart}` : t('写周报', 'Write report')} subtitle={report?.status === 'submitted' ? t('正文已提交，仅可由授权评审人填写评分。', 'Submitted contents are locked. Authorized reviewers can enter feedback.') : editable ? t('先保存草稿，核对后提交。提交后正文不可修改。', 'Save a draft, then submit when ready. Submitted contents cannot be edited.') : t('作者尚未提交，可查看当前草稿；提交后再填写评分。', 'The author has not submitted yet. You can read the draft and review it after submission.')} t={t} busy={busy} onClose={() => { setOpen(false); setFormError(null) }}>
      <div className="dialog-body">
        <form className="work-report-form" onSubmit={event => { event.preventDefault(); void save('draft') }}>
          {!report && <div className="field-pair"><label>{t('所在周（周一）', 'Week starts on Monday')}<input name="weekStart" type="date" required value={weekStart} disabled={busy} onChange={event => setWeekStart(event.target.value)}/></label>{user.isSuperAdmin ? <label>{t('周报作者', 'Report author')}<select name="authorId" required value={authorId} disabled={busy} onChange={event => setAuthorId(event.target.value)}><option value="">{t('选择成员', 'Choose a member')}</option>{members.filter(member => member.company).map(member => <option key={member.id} value={member.id}>{member.name} · {member.company}</option>)}</select></label> : <p>{user.name} · {user.company}</p>}</div>}
          <h3>{t('上周工作', 'Last week’s work')}</h3>
          ...
          <label>{t('下周计划', 'Next week’s plan')}<textarea name="nextPlan" readOnly={!editable} maxLength={4000} value={nextPlan} disabled={busy} onChange={event => setNextPlan(event.target.value)}/></label>
        </form>
      ...
      </div><footer><button disabled={busy} onClick={() => { setOpen(false); setFormError(null) }}>{t('关闭', 'Close')}</button>{editable && <><button disabled={busy || conflict} onClick={() => void save('draft')}>{t('保存草稿', 'Save draft')}</button><button className="primary" disabled={busy || conflict} onClick={() => void save('submitted')}>{t('确认提交周报', 'Submit report')}</button></>}</footer>
    </Dialog>}
  </>
}
