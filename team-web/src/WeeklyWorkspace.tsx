...
export function WeeklyWorkspace(props: WorkModuleProps) {
  ...
  function create() {
    setReport(null); setHistory([]); setWeekStart(currentReportWeek()); setAuthorId(user?.company ? user.id : members.find(member => member.company)?.id || '')
  ...
  return <>
    ...
    {open && <Dialog title={report ? `${report.authorName} · ${reportWeekRange(report.weekStart)}` : t('写周报', 'Write report')} subtitle={report?.status === 'submitted' ? t('正文已提交，仅可由授权评审人填写评分。', 'Submitted contents are locked. Authorized reviewers can enter feedback.') : editable ? t('先保存草稿，核对后提交。提交后正文不可修改。', 'Save a draft, then submit when ready. Submitted contents cannot be edited.') : t('作者尚未提交，可查看当前草稿；提交后再填写评分。', 'The author has not submitted yet. You can read the draft and review it after submission.')} t={t} busy={busy} onClose={() => { setOpen(false); setFormError(null) }}>
      ...
      </div><footer><button disabled={busy} onClick={() => { setOpen(false); setFormError(null) }}>{t('关闭', 'Close')}</button>{editable && <><button disabled={busy || conflict || (!report && !weekStart)} onClick={() => void save('draft')}>{t('保存草稿', 'Save draft')}</button><button className="primary" disabled={busy || conflict || (!report && !weekStart)} onClick={() => void save('submitted')}>{t('确认提交周报', 'Submit report')}</button></>}</footer>
    </Dialog>}
  </>
}
