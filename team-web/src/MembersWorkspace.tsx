...
import { Building2, KeyRound, HandHelping, LayoutGrid, LogOut, Package, Plus, RefreshCw, Server, Settings, ShieldCheck, Trash2, Users } from 'lucide-react'
...
import { COMPANY_OPTIONS, memberCompanies, type Company, type Member, type Session } from './types'
...
export function MembersWorkspace({ session, state, navigate, onLogout, onSessionChanged, onSessionExpired }: Props) {
  ...
  function open(value: Editor) {
    ...
    setName(''); setPassword(''); setCompanies(value.member ? memberCompanies(value.member) : [])
  }
  function close() { setEditor(null); setPassword(''); setFormError(null); setConflict(false) }
  async function save() {
    if (!editor || busy || !allowed) return
    if (editor.kind === 'create' && (!name.trim() || !password.trim() || !companies.length) || editor.kind === 'reset' && !password.trim()) {
    ...
    try {
      if (editor.kind === 'create') {
        const value = await api.createMember({ name, password, companies })
        ...
        const value = await api.setMemberCompanies(editor.member!, companies)
        if (request !== generation.current) return
        ...
        setNotice(t('所属组织已更新，成员刷新后可在已授权组织间切换。', 'Organizations updated. The member can refresh and switch between assigned organizations.'))
  ...
  const filtered = members.filter((member) => {
    const memberships = memberCompanies(member)
    const memberCompany = memberships.join('、')
    return (!companyFilter || (!member.isSuperAdmin && (companyFilter === 'pending' ? !memberships.length : memberships.includes(companyFilter as Company))))
  ...
  return <>
    <div className="workspace-shell members-workspace" inert={Boolean(editor) || settings}>
      <aside className="sidebar">
        ...
        <nav className="main-nav" aria-label={t('主导航', 'Main navigation')}><button onClick={() => navigate('/')}><LayoutGrid size={18} />{t('算力预约', 'Compute bookings')}</button><button onClick={() => navigate('/equipment')}><Package size={18} />{t('资产设备管理', 'Asset management')}</button><button onClick={() => navigate('/servers')}><Server size={18}/>{t('服务器资源', 'Server resources')}</button><button onClick={() => navigate('/requests')}><HandHelping size={18}/>{t('办公设备申请', 'Office equipment requests')}</button><button className="is-active" aria-current="page"><Users size={18} />{t('成员管理', 'Members')}</button></nav>
    ...
    {editor && <Dialog compact title={title} subtitle={editor.member ? editor.member.username === editor.member.name ? editor.member.name : `${editor.member.name} · ${editor.member.username}` : undefined} onClose={close} busy={busy} t={t}><form onSubmit={(event) => { event.preventDefault(); void save() }}><div className="dialog-body member-form">
      ...
      {['create', 'company'].includes(editor.kind) && <fieldset className="member-company-choices" disabled={busy}><legend>{t('所属组织（可多选）', 'Organizations (select multiple)')}</legend>{COMPANY_OPTIONS.map(value => <label className="checkbox-label" key={value}><input type="checkbox" name="companies" value={value} checked={companies.includes(value)} onChange={event => setCompanies(old => COMPANY_OPTIONS.filter(company => company === value ? event.target.checked : old.includes(company)))}/>{value}</label>)}<p className="field-help">{editor.kind === 'create' ? t('请至少分配一个组织。成员可在已授权组织之间切换。', 'Assign at least one organization. Members can switch between their assigned organizations.') : t('取消全部归属后，该成员将等待分配，暂时无法访问组织资料。历史记录保留。', 'Removing all memberships leaves the member waiting for assignment and disables access to organization data. History is retained.')}</p></fieldset>}
      ...
      {conflict && <div className="error" role="alert">{t('该账号刚刚有变化，已读取最新信息。你的输入已保留，请核对后再次提交。', 'This account changed. Latest details are loaded and your input is retained. Review them before submitting again.')}{editor.member && <span>{t('当前公司：', 'Current company: ')}{memberCompanies(editor.member).join('、') || t('待分配', 'Unassigned')}</span>}</div>}
