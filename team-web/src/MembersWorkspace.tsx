...
import { Activity, Building2, KeyRound, LayoutGrid, LogOut, Package, Plus, RefreshCw, Settings, ShieldCheck, Trash2, Users } from 'lucide-react'
...
export function MembersWorkspace({ session, state, navigate, onLogout, onSessionChanged, onSessionExpired }: Props) {
  ...
  function open(value: Editor) {
    ...
    setUsername(''); setName(''); setPassword(''); setCompany(value.member?.company || '')
  }
  function close() { setEditor(null); setPassword(''); setFormError(null); setConflict(false) }
  async function save() {
    if (!editor || busy || !allowed) return
    if (editor.kind === 'create' && (!username.trim() || !name.trim() || !password.trim() || !company) || editor.kind === 'company' && !company || editor.kind === 'reset' && !password.trim()) {
    ...
    try {
      if (editor.kind === 'create') {
        const value = await api.createMember({ username, name, password, company: company as Company })
  ...
  const filtered = members.filter((member) => (!companyFilter || (companyFilter === 'pending' ? !member.company : member.company === companyFilter)) && (!recoveryOnly || Boolean(member.recoveryRequestedAt)) && [member.name, member.username, member.company || ''].some((value) => value.toLocaleLowerCase().includes(search.trim().toLocaleLowerCase())))
  ...
  return <>
    <div className="workspace-shell members-workspace" inert={Boolean(editor) || settings}>
      <aside className="sidebar">
        ...
        <nav className="main-nav" aria-label={t('主导航', 'Main navigation')}><button onClick={() => navigate('/')}><LayoutGrid size={18} />{t('资源看板', 'Resource board')}</button><button onClick={() => navigate('/equipment')}><Package size={18} />{t('设备管理', 'Equipment')}</button><button className="is-active" aria-current="page"><Users size={18} />{t('成员管理', 'Members')}</button></nav>
        <div className="sidebar-bottom"><div className="sidebar-note"><ShieldCheck size={18} /><p>{t('成员名册与账号管理仅超级管理员可访问。', 'Only the super administrator can access the member directory and account management.')}</p></div><button className="sidebar-settings" onClick={() => setSettings(true)}><Settings size={17} />{t('设置', 'Settings')}</button><div className="profile"><span className="profile-avatar"><ShieldCheck size={19} /></span><div><strong>{session.user!.name}</strong><small>{t('超级管理员', 'Super administrator')}</small></div><button className="icon-button" aria-label={t('退出登录', 'Sign out')} disabled={busy} onClick={async () => { generation.current++; setMembers([]); setBusy(true); try { await onLogout() } catch (reason) { setError(reason) } finally { setBusy(false) } }}><LogOut size={16} /></button></div></div>
      </aside>
      <main className="main-workspace"><header className="page-header"><div><p>{t('团队账号', 'Team accounts')}</p><h1>{t('成员管理', 'Members')}</h1><span>{t('分配公司、处理找回申请和管理员工账号。', 'Assign companies, handle recovery requests and manage employee accounts.')}</span></div><div className="header-actions"><button disabled={loading || busy} onClick={() => void load()}><RefreshCw size={16} />{t('刷新', 'Refresh')}</button><button className="primary" onClick={() => open({ kind: 'create' })}><Plus size={17} />{t('增加员工', 'Add employee')}</button></div></header>
        <div className="page-content">
          ...
          {loading ? <p className="loading-inline" role="status"><RefreshCw size={17} />{t('正在读取成员…', 'Loading members…')}</p> : !error && <section className="member-directory" aria-label={t('团队成员名册', 'Team member directory')}><div className="member-directory-heading"><strong>{t('成员', 'Members')} <span className="muted">{filtered.length}</span></strong><span>{t('公司由超级管理员分配', 'Companies are assigned by the super administrator')}</span></div>{filtered.length ? <div className="member-table-scroll" tabIndex={0} role="region" aria-label={t('成员表，可横向滚动', 'Members table, horizontally scrollable')}><table className="member-table"><thead><tr>{[t('姓名 / 用户名', 'Name / Username'), t('公司', 'Company'), t('身份', 'Role'), t('注册时间', 'Registered'), t('找回申请', 'Recovery request'), t('操作', 'Actions')].map((value) => <th scope="col" key={value}>{value}</th>)}</tr></thead><tbody>{filtered.map((member) => <tr key={member.id}><td><strong>{member.name}</strong><span className="member-username">{member.username}</span></td><td>{member.company || <span className="member-pending">{t('待分配公司', 'Company pending')}</span>}</td><td>{member.isSuperAdmin ? t('超级管理员', 'Super administrator') : member.role === 'admin' ? t('资源管理员', 'Resource administrator') : t('团队成员', 'Team member')}</td><td><time dateTime={member.createdAt}>{formatTime(member.createdAt, preferences.locale)}</time></td><td>{member.recoveryRequestedAt ? <span className="member-pending">{t('待处理', 'Pending')}<time dateTime={member.recoveryRequestedAt}>{formatTime(member.recoveryRequestedAt, preferences.locale)}</time></span> : <span className="muted">—</span>}</td><td><div className="member-actions"><button aria-label={`${t('分配公司', 'Assign company')} · ${member.name}`} onClick={() => open({ kind: 'company', member })}><Building2 size={15} />{t('公司', 'Company')}</button>{!member.isSuperAdmin && <><button aria-label={`${t('重置密码', 'Reset password')} · ${member.name}`} onClick={() => open({ kind: 'reset', member })}><KeyRound size={15} />{t('重置密码', 'Reset password')}</button><button className="danger" aria-label={`${t('删除账号', 'Delete account')} · ${member.name}`} onClick={() => open({ kind: 'delete', member })}><Trash2 size={15} />{t('删除', 'Delete')}</button></>}</div></td></tr>)}</tbody></table></div> : <p className="member-empty">{t('没有符合条件的成员。', 'No members match these filters.')}</p>}</section>}
        </div>
      </main>
    </div>
    {settings && <SettingsDialog state={state} session={session} onClose={() => setSettings(false)} onSessionChanged={onSessionChanged} />}
    {editor && <Dialog compact title={title} subtitle={editor.member ? `${editor.member.name} · ${editor.member.username}` : undefined} onClose={close} busy={busy} t={t}><form onSubmit={(event) => { event.preventDefault(); void save() }}><div className="dialog-body member-form">
      {editor.kind === 'create' && <><label>{t('用户名', 'Username')}<input name="username" value={username} required autoComplete="off" disabled={busy} onChange={(event) => setUsername(event.target.value)} /></label><label>{t('员工姓名', 'Employee name')}<input name="name" value={name} required autoComplete="off" disabled={busy} onChange={(event) => setName(event.target.value)} /></label></>}
      ...
      {editor.kind === 'create' && <p className="field-help">{t('用户名和密码支持中英文，填写非空内容即可。姓名作为团队内的固定身份，不能与其他成员重复。', 'Username and password support Chinese and English and only need to be non-empty. The employee name is a fixed, unique team identity.')}</p>}
