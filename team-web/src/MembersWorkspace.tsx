  ...
  return <>
    <div className="workspace-shell members-workspace" inert={Boolean(editor) || settings}>
      <aside className="sidebar">
        ...
        <nav className="main-nav" aria-label={t('主导航', 'Main navigation')}><button onClick={() => navigate('/')}><LayoutGrid size={18} />{t('算力预约', 'Compute bookings')}</button><button onClick={() => navigate('/equipment')}><Package size={18} />{t('设备管理', 'Equipment')}</button><button onClick={() => navigate('/servers')}><Server size={18}/>{t('服务器目录', 'Servers')}</button><button onClick={() => navigate('/reports')}><FileText size={18}/>{t('周报与绩效', 'Weekly reports')}</button><button onClick={() => navigate('/requests')}><HandHelping size={18}/>{t('设备申请与领取', 'Device requests')}</button><button className="is-active" aria-current="page"><Users size={18} />{t('成员管理', 'Members')}</button></nav>
