...
历史副本从启用后逐步积累。只清理本工具识别且验证成功的自动快照；手动备份和未知文件不清理。发现异常快照时停止本轮清理，避免连带删除恢复点。开始前检查新快照及恢复副本所需空间，并额外预留 1 GiB 给生产数据库与系统；空间不足则失败退出。备份目录必须监控可用空间，检查失败日志后处理问题，不能把定时器已启用等同于永远备份成功。
