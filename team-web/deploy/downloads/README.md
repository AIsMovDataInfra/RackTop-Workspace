...
`index.html` 是 [云端下载页](https://136.0.110.161/downloads/) 的静态源文件，提供 2.5.0 测试版（Pre-release）。普通用户见[下载说明](../../../docs/DOWNLOADS.md)，发布维护见[维护者入口](../../../docs/MAINTAINERS.md)。页面和安装包由 Nginx 直接提供，无需登录。

公开目录为 `/var/www/racktop-public/downloads/`。新版本使用这些文件；保留历史安装包，不覆盖旧版本附件：

- `index.html`、`install-racktop.sh`、`Ubuntu20.04-install.txt`
- `RackTop_2.5.0_linux-amd64-flatpak-offline.tar.gz`
- `RackTop_2.5.0_linux-amd64.deb`
- `RackTop_2.5.0_macos-arm64-unsigned.dmg`
- `RackTop_2.5.0_macos-x64-unsigned.dmg`
- `RackTop_2.5.0_source.tar.gz`、`LICENSE`、`NOTICE.md`、`SHA256SUMS`

Ubuntu 20.04 新套件内根目录为 `RackTop_2.5.0_flatpak_offline`，运行 `bash install.sh`，仅进行用户级安装。旧 2.2.2 简易套件内才运行 `install-racktop.sh`。固定在线入口 `/downloads/install-racktop.sh` 自动识别已有 Flatpak 的安装范围。
