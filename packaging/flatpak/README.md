...
The application bundle identifies Flathub as its runtime source. Its online first installation downloads GNOME and graphics runtimes. Ubuntu 20.04 should use the complete kit for first installation. From 2.5.0, in-app updates select the `linux-x86_64-flatpak` target from the existing Linux feed, verify the complete bundle with the embedded updater public key, then verify its ID, architecture, stable branch, GNOME 50 runtime, version and OSTree commit. The host Flatpak installs that application bundle in the current user or default system installation with `--no-deps --no-related --or-update`; the same scope is used to restart it. No DEB is downloaded for a Flatpak installation.

The older 2.2.2 application does not contain this updater. It needs the one-time upgrade described in [the download guide](../../docs/DOWNLOADS.md); that upgrade reuses installed runtimes. This repository does not publish an application remote on Flathub. `flatpak update` updates runtimes installed from Flathub; the offline kit's runtimes have no update remote and must be replaced explicitly using the local bundle commands in the Linux guide. Application updates preserve those runtime deployments. Custom system installations must still be maintained through their original installation configuration.
...
- The explicit “open setup terminal” action invokes `flatpak-spawn --host x-terminal-emulator`. Signed application updates use the same existing host bridge to call `/usr/bin/flatpak`, with separate arguments and no shell evaluation. Both require the named Flatpak D-Bus permission. Normal monitoring and integrated SSH terminals use the bundled SSH client.
