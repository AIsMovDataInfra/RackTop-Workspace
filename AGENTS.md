...
- 保留默认分支 `main` 和客户端自动更新必需的 `updater`；不得为了减少分支删除、改名或合并 `updater` 到 `main`。
- 除不同产品版本外，尽量不增加远程分支。同一版本只维护一个版本分支，功能实现、后续修复、部署与验证文档均复用该分支，不再分别创建功能、验收或文档分支。
- 开始工作前先检查远程分支、版本和提交关系；已有对应版本分支时直接复用。新版本确需分支时，优先使用 `codex/vX.Y.Z`，已有带功能后缀的版本分支继续沿用，不仅为统一名称而另建分支。
- 仅修改工作规范或补充当前版本交付记录时，沿用当前版本分支，不新增产品版本或单独创建远程分支。确需并行隔离的临时分支在工作完成后及时清理。
- 清理前确认提交已完整保留于要保留的分支或标签，且没有未处理的 PR、活动工作树或自动化依赖。不同版本可以各保留一个分支；不为减少数量丢弃独有提交、改写历史或移动已发布标签。
- 本约定优先于下文按每个独立需求新建分支的旧流程。
...
3. 优先复用当前产品版本已有的分支；仅在新版本尚无对应分支时创建一个版本分支，例如：
