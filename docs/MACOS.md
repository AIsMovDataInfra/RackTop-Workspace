...
**发布状态：1.30.0 Mac 测试版已发布。** 提供两种 DMG、对应更新归档、源码、许可证及 `SHA256SUMS`，见 [v1.30.0 Release](https://github.com/AIsMovDataInfra/RackTop/releases/tag/v1.30.0)。
...
- **团队预约**：连接同一个在线预约服务，用用户名和密码注册或登录；普通成员需由超级管理员分配公司后使用。无需安装 RackTop 也可以在浏览器查看排期和预约；管理员可在桌面端将选中的服务器 GPU 清单同步到中央目录。同步不上传 SSH 地址或私钥。预约属于排期协调，不会自动授予 SSH 权限，也不会强制占用 GPU。详细流程见 [团队预约使用说明](../team-web/README.md)。
...
1.30.0 提供完整的固定资产表格标签，包含公司名称、资产编号、资产名称、责任人、使用人及二维码，可下载或打印。超级管理员通过网页管理员工、分配公司和处理密码找回申请，桌面登录也提供忘记密码入口。成员分配公司后刷新即可继续使用设备与预约，详见 [团队账号管理](TEAM_ACCOUNTS.md)。
