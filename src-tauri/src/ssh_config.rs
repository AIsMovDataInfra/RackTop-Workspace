...
pub fn parse(content: &str) -> Vec<ServerDraft> {
    ...
    fn finish(block: HostBlock, output: &mut Vec<ServerDraft>) {
        for alias in block.aliases.into_iter().filter(|value| !value.contains('*') && !value.contains('?') && !value.starts_with('!')) {
            ...
            output.push(ServerDraft { id: None, name: alias.clone(), location: None, host: hostname, port: block.port.unwrap_or(22), username: block.user.clone().unwrap_or_else(default_username), ssh_alias: Some(alias), identity_file: block.identity_file.clone(), proxy_jump: block.proxy_jump.clone(), proxy_use_password: false, tags: Vec::new(), sampling_interval_seconds: 2, history_retention_days: 90, remote_history_enabled: false, auth_method: "sshConfig".into(), password: None, save_password: false, proxy_password: None, save_proxy_password: false });
        }
    }
