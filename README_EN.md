...
The current source version is the **2.2.0 release candidate**, targeting Ubuntu 22.04 amd64 and both Apple Silicon and Intel Macs. It fixes vertically clipped native selects throughout the desktop; lets one invitation enroll multiple devices before expiry while keeping each device independently authorized and rotating the invitation when a member is revoked; and shows the owner an online guest's public egress IP with an explicit NAT, VPN, and proxy caveat. The 2.2.0 tag, public installers, update feeds, production relay deployment, and local installation are still pending. The latest downloadable and deployed version remains [2.1.1](https://github.com/AIsMovDataInfra/RackTop-Workspace/releases/tag/v2.1.1); see its [verification record](docs/VERIFICATION_2_1_1.md).
...
**2.2.0 is still a release candidate and has no public installers yet.** Use the verified 2.1.1 pre-release below for now. Its anonymous downloads, GitHub digests, SHA256 values, source correspondence, and updater signatures have been checked, and both Mac architectures are available. See the [2.1.1 checksum table](docs/VERIFICATION_2_1_1.md#公开附件与更新签名).
...
The following is the previous **2.0.0 verification snapshot**, not a 2.1.0 result. Desktop frontend tests (322), web tests (91), backend tests (132 plus four additional security regressions), Linux Rust tests (160 passed, three external-environment tests ignored), and both production frontend builds have passed. All three platform jobs and publication in the [tag workflow](https://github.com/AIsMovDataInfra/RackTop-Workspace/actions/runs/34294527671) succeeded. Nine public assets, the 414-file source archive, and all three updater signatures were verified. Native CI startup and both Mac screenshots passed review. The local Linux user installation is now 2.0.0 with its previous data preserved; the existing process has not been restarted. System-wide installation and full user-device interaction are separate from this user-directory installation. Production migration, HTTPS, anonymous business-access rejection and existing-administrator sign-in passed. See the [2.0.0 acceptance record](docs/VERIFICATION.md); previous checks remain historical records in [the Linux guide](docs/LINUX.md).
...
- **Resource sharing**: Use authorized monitoring, terminal, and file capabilities through the owner's computer without receiving its SSH private key. The 2.2.0 candidate supports independently authorized multi-device enrollment from one invitation and shows the owner an online guest's public egress IP.
