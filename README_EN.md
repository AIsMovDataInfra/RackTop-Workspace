...
The current distribution is **2.0.0** for Ubuntu 22.04 amd64, macOS Apple Silicon and macOS Intel, sharing one version and Release. All three platform builds and publication succeeded; the public [v2.0.0 assets](https://github.com/AIsMovDataInfra/RackTop-Workspace/releases/tag/v2.0.0), checksums and updater signatures have been verified, and verification is recorded in the [acceptance record](docs/VERIFICATION.md). The online workspace was deployed on 2026-09-09; migration checks and sign-in with the existing administrator passed. The previous repository and its update feeds remain unchanged. Existing users must manually install a new package once; the application identity and local data stay compatible.
