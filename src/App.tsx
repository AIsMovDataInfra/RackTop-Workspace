...
function serverToDraft(server: Server): Partial<ServerDraft> {
  return {
    ...
    proxyUsePassword: server.proxyUsePassword ?? false,
    saveProxyPassword: server.saveProxyPassword ?? false,
    tags: server.tags,
...
function App() {
  ...
  async function saveServer(draft: ServerDraft) {
    ...
    const managedGeneration = previous?.managed ? managedGenerations.current.get(previous.id) ?? 0 : null
    const saved = await api.saveServer(draft)
    // A directory event may revoke this connection while the save response is
    // in flight. Do not restore its old availability or begin another SSH call.
    if (managedGeneration !== null && managedGeneration !== (managedGenerations.current.get(previous!.id) ?? 0)) return
    const server = saved
  ...
  return (
    <div className="app-shell">
      ...
      {pendingHostKey && <HostKeyDialog info={pendingHostKey} onClose={() => setPendingHostKey(null)} onTrust={async () => { const serverId = pendingHostKey.serverId; await api.trustHostKey(pendingHostKey); setPendingHostKey(null); setToast(pendingHostKey.isProxy ? '已信任跳板机指纹' : '已信任服务器指纹'); await refreshServer(serverId) }} />}
