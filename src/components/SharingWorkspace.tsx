...
export function SharingWorkspace({ servers, currentServerId }: { servers: Server[]; currentServerId?: string | null }) {
  ...
  return <div className="sharing-workspace">
    ...
    {loading ? <div className="sharing-empty" role="status">正在读取共享状态…</div> : page === 'owner' ? <>
      ...
      <div className="sharing-owned-list">{status.shares.map(share => {
        ...
        return <section className="sharing-owned-card" key={share.id}>
          ...
          <div className="sharing-actions"><button className="button button--primary" disabled={!!busy || share.paused || expired || !status.ownerOnline} onClick={() => void run('读取邀请码', () => issueInvite(share.id))}><Link2 size={15}/>查看邀请码</button><button className="button button--secondary" disabled={!!busy || expired} onClick={() => void run('更新共享状态', () => sharingApi.pause(share.id, !share.paused))}>{share.paused ? <Play size={14}/> : <Pause size={14}/>} {share.paused ? '恢复共享' : '暂停共享'}</button><button className="button button--secondary sharing-danger" disabled={!!busy} onClick={() => showConfirm('删除这项分享？', '所有成员将失去访问权限。原服务器配置和远端文件会保留。', () => sharingApi.delete(share.id))}><Trash2 size={14}/>删除分享</button></div>
          <div className="sharing-members"><h3><Users size={15}/>已授权设备 <span>{share.members.length}</span></h3>{share.members.length ? <>{share.members.map(member => { const ipAddress = member.ipAddress?.trim(); return <div className="sharing-member" key={member.id}><div><strong>{member.deviceName}</strong><small>{member.connected ? '已连接' : `最近访问 ${date(member.lastSeenAt)}`} · {date(member.pairedAt)} 加入</small>{member.connected && ipAddress && <small className="sharing-member__ip">公网出口 IP <code>{ipAddress}</code></small>}</div><button className="button button--secondary" disabled={!!busy} onClick={() => showConfirm(`撤销「${member.deviceName}」？`, '该设备的会话将断开，现有邀请码会自动更新；其他已授权设备保持可用。已脱离终端的远端后台任务不会被终止。', () => sharingApi.revokeMember(share.id, member.id))}>撤销访问</button></div> })}{share.members.some(member => member.connected && member.ipAddress?.trim()) && <p className="sharing-hint sharing-network-hint">公网出口 IP 是设备连接中继时使用的网络出口地址。同一 NAT 网络中的多台设备可能显示相同 IP，不能只凭它识别具体设备。</p>}</> : <p className="sharing-hint">还没有成员。同一个有效邀请码可供多台设备加入，每台设备独立授权。</p>}</div>
        </section>
      })}</div>
    </> : <>
    ...
    {invite && <Modal title="邀请协作者" onClose={() => setInvite(null)}><div className="sharing-form"><p>此邀请码在过期前可反复复制，并可供多台设备分别加入。撤销任一设备后，邀请码会自动更新。</p><textarea aria-label="共享邀请码" readOnly value={invite.code} rows={4} data-sharing-autofocus/><p className="sharing-hint">{invite.expiresAt <= now ? '邀请码已过期，请重新获取。' : `${date(invite.expiresAt)} 过期`}</p><div className="sharing-form-actions"><button className="button button--secondary" onClick={() => setInvite(null)}>完成</button><button className="button button--primary" disabled={invite.expiresAt <= now} onClick={() => { void navigator.clipboard.writeText(invite.code).then(() => setCopied(true)).catch(() => setError('复制失败，请选中邀请码并手动复制。')) }}>{copied ? <Check size={15}/> : <Copy size={15}/>} {copied ? '再次复制' : '复制邀请码'}</button></div></div></Modal>}
