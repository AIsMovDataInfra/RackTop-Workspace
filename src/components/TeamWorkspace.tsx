...
export function TeamWorkspace({ servers, snapshots }: { servers: Server[]; snapshots: Record<string, Snapshot> }) {
  ...
  return <div className="team-workspace">
    ...
    {admin && <section className="team-panel"><div className="team-section-title"><div><h2>同步本机资源</h2><p>每 30 秒同步已选个人服务器的硬件资源。团队服务器资源单独管理；取消勾选会停止上报，已有在线资源及预约不会被删除。</p></div><button className="button button--primary" disabled={!!busy} onClick={() => void run('同步中', synchronize)}>{busy === '同步中' ? busy : dirty ? '保存并同步' : '立即同步'}</button></div>
