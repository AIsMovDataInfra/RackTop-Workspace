...
export function TeamWorkspace({ servers, snapshots }: { servers: Server[]; snapshots: Record<string, Snapshot> }) {
  ...
  const [data, setData] = useState<TeamData>({ resources: [], reservations: [] })
  ...
  const admin = status.authenticated && status.user?.role === 'admin'

  async function refresh() {
    const current = await teamApi.status()
    setStatus(current)
    if (!dirty) setSelected(Object.keys(current.bindings))
    setData(await teamApi.data())
  }
  useEffect(() => {
    let active = true
    const poll = async () => {
      try {
        const current = await teamApi.status(); if (!active) return
        setStatus(current)
        if (!dirty) setSelected(Object.keys(current.bindings))
        const next = await teamApi.data(); if (active) setData(next)
      } catch (e) { if (active) setError(String(e)) } finally { if (active) setLoading(false) }
    }
    void poll()
    const interval = window.setInterval(() => { if (!document.hidden && !busy) void poll() }, 30_000)
  ...
  async function signOut() {
    ...
    finally { try { setStatus(await teamApi.status()) } catch (e) { setError(String(e)) }; setLoggingOut(false) }
  }
  ...
  return <div className="team-workspace">
    ...
    {loading && <p className="team-footnote" role="status">正在读取在线预约数据…</p>}
    {error && <div className="team-error" role="alert">{error}</div>}
    {notice && <div className="team-notice" role="status"><Check size={16}/>{notice}</div>}
    <div className="team-account"><div><strong>{status.authenticated ? status.user?.name : '尚未连接团队账号'}</strong><p>{status.authenticated ? `${status.user?.username} · ${admin ? '管理员' : '团队成员'}` : '查看排期无需登录；管理员登录后可以同步本机资源。首次使用请在网页注册。'}</p></div>
      {status.authenticated ? <button className="button button--secondary" disabled={loggingOut} onClick={() => void signOut()}><LogOut size={15}/>退出账号</button> : <button ref={loginButton} className="button button--secondary" onClick={() => setLogin(true)}><LogIn size={15}/>账号登录</button>}
    </div>
    {admin && <section className="team-panel"><div className="team-section-title"><div><h2>同步本机资源</h2><p>每 30 秒同步已选资源。取消勾选会停止上报，已有在线资源及预约不会被删除。</p></div><button className="button button--primary" disabled={!!busy} onClick={() => void run('同步中', async () => { const current = await teamApi.select(selected); setStatus(current); setDirty(false); setData(await teamApi.data()); setNotice(!current.authenticated ? '已停止本机资源同步。' : Object.values(current.bindings).some(b => b.error) ? '已保存选择；部分资源需要处理下方提示。' : '资源已同步，网页与桌面使用同一份资源目录。') })}>{busy === '同步中' ? busy : dirty ? '保存并同步' : '立即同步'}</button></div>
    ...
    }}><div className="team-section-title"><h2 id="team-login-title">连接团队账号</h2><button className="icon-button" aria-label="关闭登录" disabled={!!busy} onClick={() => {setLogin(false);setPassword('')}}><X size={18}/></button></div><form onSubmit={e => { e.preventDefault(); void run('登录中', async () => { const current=await teamApi.login(username,password);setPassword('');setStatus(current);setSelected(Object.keys(current.bindings));setDirty(false);setLogin(false);setData(await teamApi.data());setNotice('团队账号已连接。') }) }}><label>用户名<input type="text" autoComplete="username" value={username} onChange={e=>setUsername(e.target.value)} minLength={3} maxLength={32} pattern="[A-Za-z0-9_-]+" required disabled={!!busy}/></label><label>密码<input type="password" autoComplete="current-password" value={password} onChange={e=>setPassword(e.target.value)} required maxLength={128} disabled={!!busy}/></label><p>使用预约网页注册的账号。登录凭据保存在本机系统钥匙串。</p>{error && <div role="alert" className="team-error">{error}</div>}<button className="button button--primary" disabled={!!busy}>{busy || '登录'}</button><button type="button" className="button button--secondary" onClick={()=>void openWeb()}>打开网页注册</button></form></div></div>}
  </div>
}
